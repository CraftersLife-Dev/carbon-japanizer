...
/**
 * <p>{@link ConfigurateException}をラップし、チェックされない例外として扱うためのランタイム例外。</p>
 *
 * <p>このクラスは、{@code ConfigurateException}がチェックされる例外であるために、
 * 呼び出し元で強制的に{@code try-catch}ブロックを記述する必要があるのを避けるために使用される。
 * これにより、{@code ConfigurateHelper}のようなユーティリティクラスをより簡潔に使用できるようになる。</p>
 */
@NullMarked
public final class UncheckedConfigurateException extends RuntimeException {
    ...
    /**
     * 指定された{@link ConfigurateException}を原因として、新しい{@code UncheckedConfigurateException}を構築する。
     *
