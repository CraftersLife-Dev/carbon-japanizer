 ...
 */
package io.github.crafterslife.dev.carbonjapanizer.resource.translation;

import io.github.crafterslife.dev.carbonjapanizer.utility.JISSafetyColorMiniMessageFactory;
...
public final class TranslationStoreInitializer {
    ...
    public static List<Locale> initialize(
        ...
        final Key key = Key.key("carbon_japanizer", baseName);
        final MiniMessageTranslationStore store = MiniMessageTranslationStore.create(key, JISSafetyColorMiniMessageFactory.create());
        store.defaultLocale(Locale.JAPAN);

        final ClassLoader classLoader = TranslationStoreInitializer.class.getClassLoader();
