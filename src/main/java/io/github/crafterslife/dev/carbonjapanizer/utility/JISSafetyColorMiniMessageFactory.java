 ...
 */
@NullMarked
public final class JISSafetyColorMiniMessageFactory {

    @Contract(pure = true)
    private JISSafetyColorMiniMessageFactory() {
    }

    /**
     * カスタムタグをサポートするMiniMessageインスタンスを生成する。
     *
     * <p>このインスタンスは、標準的なMiniMessageタグに加えて、特定の意味を持つ
     * カスタムカラータグ（{@code <error>}、{@code <warn>}、{@code <info>}、{@code <debug>}）を
     * 解決する。これらのタグは、JIS安全色に基づいており、どのような環境下でも認識しやすいユニバーサルデザインカラーが採用されている。</p>
     *
     * @return カスタムタグが登録された{@link MiniMessage}インスタンス
     */
    public static MiniMessage create() {

        final TagResolver customTagResolver = TagResolver.builder()
                .resolver(TagResolver.standard())
                .resolver(JISSafetyColorMiniMessageFactory.createJISSafetyColors())
                .build();

        return MiniMessage.builder()
                .tags(customTagResolver)
                .build();
    }

    private static TagResolver createJISSafetyColors() {
